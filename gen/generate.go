...
func generate(cfg config) (code string, err error) {
	imports := []Code{
		...
		Import("aspect", "", "github.com/CherkashinEvgeny/goaspect"),
	}
...
func generateInterfaceAspect(pkg *types.Package, aspectCfg aspectConfig) Code {
	...
	aspectType := Type(aspectCfg.AspectName, Struct(FieldDecls(
		...
		FieldDecl("container", Ptr(Named("github.com/CherkashinEvgeny/goaspect", "Container"))),
	)))
...
func generateAspectMethodBody(
	...
	returnResult := Join(Return, Raw(" "), Ids(resultsNames...))
	return Lines(
		aspect,
		before,
		implCall,
